...
/*品质生活动画*/
 ...
 //console.log(pzsh)
  ...
  function donghua(mgs){
    ...
    animate(mgs,{marginLeft:-10},300)
   }
   mgs.onmouseout=function(){
    animate(mgs,{marginLeft:0},300)
   }
   }
/*选项卡*/ 
...
/*//小轮播
  function weel(xb){
    ...
    var aw=parseInt(getStyle(as[0],'width'));
      //alert(aw)
    weel.style.width=as.length*aw+'px';
    //console.log(boxs)
    var num=0;
    var t=setInterval(wel,2000)
    function wel(){
    	num++;
        if(num>as.length-1){
        	num=0;
    }
        animate(weel,{marginLeft:-aw*num},300);
        for(var i=0;i<as.length;i++){
        	btns[i].className="";
        }
        ...
        for(var i=0;i<btns.length;i++){
        	...
        	btns[i].onmouseover=function(){
             ...
             btns[this.ind].className="hot";
             animate(weel,{marginLeft:-aw*this.ind},300);
            num=this.ind;
        	}
         }
       }
...
/*楼层跳转*/
  ...
  for(var i=0;i<lcs.length;i++){
    lcarr.push(lcn[i].offsetTop-150);
    lcs[i].lctz=i;
    lcs[i].onclick=function(){
    animate(document.documentElement,{scrollTop:lcarr[this.lctz]})
    animate(document.body,{scrollTop:lcarr[this.lctz]})
    }
  }
  window.onscroll=function(){
     var doc=document.body.scrollTop?document.body.scrollTop:document.documentElement.scrollTop;
     //console.log(doc)
     for(var i=0;i<lcarr.length;i++){
        if(doc>=lcarr[i]){
          for(var j=0;j<lcs.length;j++){
          nums[j].style.display="block";
          lcms[j].style.display="none"; 
          }
          nums[i].style.display="none";
          lcms[i].style.cssText="color:#C81623";
          lcms[i].style.display="block";
        }
     }
     if(doc<lcarr[0]||doc>lcarr[10]+500){
